...
SELECT cms_admin.* FROM cms_admin; #适用于多表查询，
...
SELECT id AS proId,proId AS id,username FROM cms_user; #尽量不要省略as
...
-- 查询表中记录age值为NULL
SELECT * FROM cms_user WHERE age=NULL; #不能用＝检测值是否为null

SELECT * FROM cms_user WHERE age<=>NULL; #<=>可以检测null值 

SELECT * FROM cms_user WHERE age<=>18;

-- IS NULL 或者IS NOT NULL
SELECT * FROM cms_user WHERE age IS NULL;
#查询值是否为NULL的行有两种方法：1:<=>NULL 2: IS NULL

#####################################
-- 范围查询 BETWEEN AND
-- 查询编号在3~10之间的用户 ----- BETWEEN AND
SELECT * FROM cms_user WHERE id BETWEEN 3 AND 10;

-- 查询编号为1，3，5，7，9，11，13，100 ---指定集合 IN
...
SELECT * FROM cms_user WHERE username IN('KinG','QUEEN','张三','章子怡');#查询时忽略大小写
...
SELECT * FROM cms_user WHERE username LIKE '%';#查询所有条件
...
-- 简单分组查询：GROUP BY
-- 按照用户所属身份分组proId
SELECT * FROM cms_user GROUP BY proId; #只会显示出组中第一个数据
...
########################################
-- 分组查询配合聚合函数 COUNT,MAX,MIN,AVG,SUM
...
MIN(age) AS min_age
FROM cms_user
GROUP BY sex WITH ROLLUP; #附加一栏显示总的信息
...
##################################################
-- having 字句对分组结果进行二次筛选，与where类似，但只能对分组过后的筛选
-- 查询组中人数大于2的
SELECT sex,GROUP_CONCAT(username) AS users,
COUNT(*) AS totalUsers,
MAX(age) AS max_age,
